...
class HighlightSideBar extends Component {
  ...
  handleScrollEvent = () => {
    ...
    const offsetAdjustment = this.props.offsetAdjustment;

    document
      .getElementById(`${anchorsOffset[0].id}-menu`)
      .classList.add(cx('current'));
    // add scroll event
    document.getElementsByClassName(cx('children'))[0].addEventListener(
      ...
      _.throttle(() => {
        ...
        for (const item of anchorsOffset) {
          document
            .getElementById(`${item.id}-menu`)
            .classList.remove(CLASSNAME);
        }
        // get current offset when scroll
        const currentOffset = document.getElementsByClassName(cx('children'))[0]
          .scrollTop;
        if (
          currentOffset + offsetAdjustment >
          anchorsOffset[anchorsOffset.length - 1].offset
        ) {
          document
            .getElementById(
              `${anchorsOffset[anchorsOffset.length - 1].id}-menu`
            )
